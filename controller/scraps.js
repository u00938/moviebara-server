      ...
      if (err) {
        ...
        return new Promise((resolve, reject) => {
          ...
          // 데이터가 존재할 경우, 이미 있는 데이터라고 메시지 보내줌
            res.status(400).send ({ data: null, message: "Data already exists"})
          }
        })
      }
    })
          ...
          try {
            ...
            if (query.user_id) {
              ...
              const scrapByUser = await scrap.findAll({
                ...
                include: [
                  { model: post,
                    attributes: ["id", "text", "rate", "createdAt", [sequelize.fn("COUNT", sequelize.col("post.scraps.id")), "scrap"]],
                    include: [
              ...
              // 성공적으로 가져왔다면, 응답에 전송
              if (scrapByUser) res.status(200).json({ data: scrapByUser, message: "ok"})
              // 가져오지 못했다면 에러메시지
              else res.status(400).json({ message: "There's no scrap post of user" })
            }
    ...
    jwt.verify(req.cookies.accessToken, process.env.ACCESS_SECRET, async (err, result) => {
      ...
      if (err) {
        ...
        return new Promise((resolve, reject) => {
          ...
          // 데이터 삭제
          const scrapDelete = await scrap.destroy({ where: { userId, postId } })
          if(scrapDelete) {
            // 성공적으로 삭제된 경우 응답에 전송
            res.status(200).json({ data: scrapDelete, message: "delete success" })
          }
          // 삭제가 되지 않을 경우 에러메시지
          res.status(400).json({ message: "There's no such data" })
        })
      }
    })
  } catch(err) {
